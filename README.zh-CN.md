...
- 按路由限制来源 IP（单地址、CIDR，或多个地址逗号分隔；空或 `0.0.0.0/0` 表示不限制）；
- 校验 Yutrix API Key；
- 替换上游供应商 API Key；
- 改写请求中的 `model`；
- 处理并发排队；
...
- **路由来源限制 (Source IP Restriction)**：每条路由规则可按客户端来源 IP 做白名单。字段留空或填写 `0.0.0.0/0`（以及 `::/0`）表示不限制；配置后支持单个 IPv4/IPv6、CIDR 子网（如 `192.168.1.0/24`），以及逗号分隔的多项（如 `192.168.1.0/24, 10.0.0.1, 203.0.113.195`）。未命中的请求在进入上游、漏斗降级和 Token 计量之前直接 `403`。已放行的请求仍走原有多级漏斗 / EmptyOutput / 429 降级路径。动作日志会带上访问者 IP，可在实时日志页按 IP 检索。
...
- **工具循环熔断**：网关会硬停反复失败的 Agent 工具续写——同一错误默认连续 5 次，或两个错误 A-B 乒乓默认 8 个半周期；另有距上次真实用户消息 400 轮 / 2 小时的兜底。返回 HTTP 200 且 OpenAI `finish_reason: stop` / Anthropic `end_turn`（不 429、不在循环中途换模型）。管理员可在 **设置 → 会话与网关设置** 开关熔断并调整这四个数字；轮数上限和最长时长填 `0` 表示关闭该信号。非法值回落到出厂默认；读取设置失败则 fail-open（不硬停）。

...
describe("OpenRouter effectiveUpstreamProtocol logic", () => {
  it("routes Anthropic→OpenAI OpenRouter stream to adapted forwarder when only OpenAI URL is bound", async () => {
    // OpenAI-bound OpenRouter: effective protocol is openai, so client Anthropic
    // SSE must be re-emitted via the adapted (cross-protocol) forwarder.
  ...
  it("routes native Anthropic OpenRouter stream to transparent forwarder when anthropic URL is bound", async () => {
    const reply: any = { raw: { write: vi.fn() } };

...
export const KNOWN_BUG_BRAND_SOURCE =
  String.raw`\bbug[-_]?(?:analyzer|locator|snag|tracker|bot|finder)\b|\bbug\s+(?:analyzer|locator|snag|tracker|bot|finder)\b`;
...
function hasKnownBugBrand(n: string): boolean {
  return knownBugBrandRe().test(n) || /\bbug(?=定位|分析|助手|工具|系统)/.test(n);
}
...
export function looksLikeStructuredLogPaste(raw: string): boolean {
  ...
  const kv = (t.match(/\b(?:function|class|return|status|level|msg|message|request_id|trace_id)=/gi) || [])
    .length;
  if (kv >= 3) return true;
...
export function hasStrongCodeDevIntent(normalizedInput: string, rawText?: string): boolean {
  ...
  if (/\bsrc[/\\]|\.(tsx?|jsx?|vue|java|py|go|rs|sql|cpp|c|cs|rb|php|kt|swift)\b/.test(n)) {
    return true;
  }
  ...
  if (
    ...
    /\b(api|endpoint|parser|function|component|service)\b.{0,40}\bimplement\b/.test(n)
  ) {
    return true;
  }
  if (/\b(refactor|write a (?:unit )?test|add a (?:method|function|component))\b/.test(n)) {
    return true;
  }
  if (/实现|重构|写一个|新增.*功能|封装/.test(n) && /接口|组件|函数|样式|页面|api|css|代码/.test(n)) {
    return true;
  }
  ...
  const fence = /```/.test(raw) || /^(def |fn |func |package |SELECT )/m.test(raw);
  if (codeTokens >= 8 || (raw.length > 2000 && codeTokens >= 4) || (fence && codeTokens >= 3)) {
    return true;
  }
...
export function hasLiveBreakageSignal(normalizedInput: string): boolean {
  ...
  const exceptionDevFraming =
    ...
    /\b[a-z0-9_]*exception\b.{0,32}\b(behavior|unit\s*test|documentation|handling|and retry)\b/.test(n) ||
  ...
  if (
    ...
    /\b(keeps? timing out|timing out|timed out|times out|time out|requests? time out)\b/.test(n) ||
    /\b(returns?|returned)\s*(?:a\s+)?(?:5\d\d|500|502|503|504)\b|\bhttp\s*5\d\d\b/.test(n) ||
    ...
    /\berror\s+occurred\b|\bshows?\s+an?\s+error\b|\bhas\s+an?\s+error\b/.test(n)
  ) {
  ...
  if (/(新增|添加|实现).{0,12}(报错|异常|堆栈).{0,12}(提示|展示|功能)/.test(n)) {
    return false;
  }
  if (
    ...
    (/(不存在|无效)/.test(n) && !/当.+不存在|若不存在|is not found|when .+ not found|token is invalid/.test(n))
  ) {
    return true;
  }
  ...
  if (/滚动条|盖住|溢出|重叠|遮挡|错位/.test(n)) {
    ...
    if (
      /^(?=.*(加|添加|支持|写|示例|布局|放在|需求)).*(滚动条|溢出|重叠)/.test(n) &&
      !/(还是|仍然|又|再次|出现).{0,8}(错位|溢出|重叠|遮挡|盖住)/.test(n)
    ) {
      // "需求定了，滚动条放在右边" — 了 is not fault tone
      if (/(放在|加一个|支持|示例|布局)/.test(n) && !/(还是|仍然|错位|盖住|bug|报错)/.test(n)) {
        return false;
      }
    }
    // Fault after feature: 实现溢出处理后页面还是错位
    if (/(还是|仍然|又|再次).{0,12}(错位|溢出|重叠|遮挡|盖住|不)/.test(n)) return true;
    if (/(出现|盖住).{0,8}(溢出|重叠|遮挡|滚动)/.test(n)) return true;
    // production path + layout fault
    if (/\.(vue|tsx?|jsx?)\b/.test(n) && /(溢出|错位|遮挡|盖住)/.test(n)) return true;
    return false;
  }
...
export function hasExplicitFailureSignal(normalizedInput: string): boolean {
  ...
  if (/还是失败|仍然失败|请求还是失败|部署失败|失败了|还是报错/.test(n)) return true;
  ...
  if (isProductStyleBugMention(n) && !hasLiveBreakageSignal(n)) {
    ...
    if (/\b(has a bug|has bugs|shows? an? error|is failing|are failing)\b/.test(n)) return true;
    return false;
  }
...
export function matchShortCnTaskAlias(normalizedInput: string): UtteranceMatch | null {
  // Keep original for punctuation strip
  let n = norm(normalizedInput);
  if (!n) return null;
  // Feature framing: not short alias
  if (/(新增|添加|实现).{0,10}(堆栈|报错|异常).{0,10}(功能|展示|提示)/.test(n)) {
    return null;
  }

  const soft = n.replace(/[。！？.!?]+$/g, "").trim();

  // Vision: optional 请, 看一下/看下/看看 + 这张/这幅图 + optional 吗 + optional short tail (布局)
  if (
    /^(请)?(看一下|看下|看看)这[张幅]图([吗么])?$/.test(soft) ||
    /^(请)?(看一下|看下|看看)这[张幅]图的(布局|内容|文字)$/.test(soft) ||
    /^你能看到图片吗$/.test(soft)
  ) {
    return { taskType: "vision", utterance: soft, score: 900, reason: "alias_vision_short" };
  }

  // Debug stack: bare or with short diagnostic tail
  if (/^堆栈信息$|^异常堆栈$|^堆栈如下$/.test(soft)) {
    return { taskType: "debug", utterance: soft, score: 900, reason: "alias_debug_short" };
  }
  if (/^堆栈信息(如下)?[：:].{0,40}$/.test(soft) && !/功能|展示|新增/.test(soft)) {
    return { taskType: "debug", utterance: soft, score: 880, reason: "alias_debug_stack_tail" };
  }

  if (/^从日志[里中]定位问题$|^看一下服务日志$/.test(soft)) {
    return { taskType: "long_context", utterance: soft, score: 900, reason: "alias_long_context_short" };
  }
  ...
  for (const taskType of TASK_ORDER) {
    ...
    for (const utt of list) {
      ...
      if (text === utt) {
        ...
        if (taskType === "long_context" && (hasStrongCodeDevIntent(text) || hasStrongWritingIntent(text))) {
          continue;
        }
...
export function isProductStyleBugMention(normalizedInput: string): boolean {
  ...
  if (/\b(has a bug|has bugs|shows? an? error|is failing|are failing)\b/.test(n)) return false;
  if (/\b(fix|investigate|resolve|repair|debug)\b/.test(n) && /\bbugs?\b/.test(stripped)) {
    return false;
  }
...
export function hasIntentBugToken(normalizedInput: string): boolean {
  ...
  if (
    /\b(fix|investigate|resolve|repair|debug)\b.{0,28}\bbugs?\b/.test(n) ||
    (/\bbugs?\b.{0,28}\b(investigate|resolve|repair)\b/.test(n) && !/\brelease notes\b|\bwrite\b/.test(n))
  ) {
    return true;
  }
  ...
  if (/\b(investigate|fix|resolve)\b.{0,16}\b(these|those|production|critical)\s+bugs\b/.test(n)) {
    return true;
  }
  if (/\b(these|those|production|critical)\s+bugs\b/.test(n) && /\b(fix|investigate|resolve|repair)\b/.test(n)) {
    return true;
  }
  ...
  if (isProductStyleBugMention(n)) {
    ...
    if (/\bbugs?\b/.test(stripped) && /\b(fix|investigate|resolve)\b/.test(n)) return true;
    return false;
  }

  const stripped = stripKnownBugBrands(n);
  // Standalone singular "bug" only with fault/work context, not bare word
  if (/\bbug\b/.test(stripped) && /\b(fix|the bug|a bug|this bug|parser)\b/.test(n)) return true;
  if (/\bbugs\b/.test(stripped) && /\b(fix|investigate|resolve|repair|these|production)\b/.test(n)) {
    return true;
  }
  return false;
}
...
export function hasLongContextLogAnalyzeSignal(normalizedInput: string): boolean {
  ...
  if (hasStrongCodeDevIntent(n) || hasStrongWritingIntent(n)) {
    ...
    if (!/\b(analyze|check|find|parse|search|summarize|inspect|review)\b.{0,24}\b(?:log|logs)\b/.test(n)
      && !/(分析|查看|检查).{0,12}日志/.test(n)) {
      return false;
    }
    if (/\b(implement|build|src[/\\]|storage api|parser api)\b/.test(n)) return false;
    if (/\bwrite an article\b|\btranslate\b/.test(n)) return false;
  }

  // Primary action instrumentation only at start / as main clause
  if (
    /^(please\s+)?(add|enable|inject)\b.{0,24}\b(?:log(?:ging)?|logs)\b/.test(n) ||
  ...
  if (new RegExp(`\\bprint\\b.{0,16}${enVerb}.{0,20}${logWord}`).test(n)) return true;
  ...
  if (new RegExp(`${enVerb}.{0,20}\\berrors?\\b.{0,16}\\b(?:nginx|docker|git|api|server)\\b.{0,8}${logWord}`).test(n)) {
    return true;
  }
  if (new RegExp(`${enVerb}.{0,16}\\b(?:nginx|docker|git|api|server|application|access)\\b.{0,10}${logWord}`).test(n)) {
    return true;
  }
  if (
    new RegExp(`\\b(?:nginx|docker|git|api|server|application|access)\\b.{0,10}${logWord}`).test(n) &&
  ...
  if (
    new RegExp(`${enVerb}.{0,24}\\b(audit(?:\\s+log)?|transcript)\\b`).test(n) ||
    new RegExp(`${zhVerb}.{0,20}(审计日志|长文本|长日志)`).test(n)
  ) {
    if (!/\b(implement|build|write an article|storage api|parser api)\b/.test(n)) return true;
  }

  // Migration docs / scripts as long content (not src/file fix tasks)
  if (/数据库迁移|迁移脚本/.test(n) && !/\bsrc[/\\]|\.ts\b|implement|fix\b/.test(n)) {
    return true;
  }
